...
np.testing.assert_array_equal(st.getAxisValues('axis1'), ['a', 'b', 'c', 'd'])

logging.info('Set new axes values')
st.setAxisValues('axis1',['e','f','g','h'])
logging.info('Get new Axis1 Values (exp: e,f,g,h)')
print(st.getAxisValues('axis1'))
np.testing.assert_array_equal(st.getAxisValues('axis1'), ['e', 'f', 'g', 'h'])
...
logging.info('Get Values Iter (exp: 10)')
i=0
for matrix, coord, sel in st.getValuesIter(returnAxes=['axis3']):
    i += 1
print(matrix.shape)
assert matrix.shape == (10,)
print("Iterations:", i, "(expected: 2x4=8)")
assert i == 8

logging.info('Get Values Iter (exp: 4x10)')
i=0
...
print(st.getHistory())
assert 'History is working.' in st.getHistory()
